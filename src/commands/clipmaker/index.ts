...
const clipmaker = async (ctx: Context): Promise<void> => {
  try {
    ...
    const meditationSteps = await getMeditationSteps({
      prompt: `Create 4 coherent steps with very short one-sentence abstracts on the topic of meditation with the addition of selling NAD+ application, where the third step must mention NAD+. Answer in json format. The structure should be as follows:
